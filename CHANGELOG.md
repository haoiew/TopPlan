...
- Windows 发布版不再显示额外的控制台窗口。
- 富文本编辑器复用统一的缩进注释解析逻辑，避免编辑器与 Markdown 视图的行映射规则发生偏差。

### 仓库配置

- 优化中英文 README 的项目定位、关键词、下载入口和真实界面预览，提升搜索与项目理解效率。
- 新增 `CITATION.cff` 标准引用元数据，并完善前端包和 Rust crate 的项目、许可证与关键词字段。
- 扩充 GitHub About、Topics 和双语 Release 说明，便于按用途、平台和技术栈检索项目。

## 0.3.0 - 2026-07-22

相对于上一版本 `0.2.0`（提交 `9bde3e0`）。
...
- 便签顶部按钮改为全屏、关闭、穿透的顺序，并增加滚动条顶部留白；双栏侧栏改为外框选中样式，两份文档的双栏图标以无背景的浅蓝线条常驻显示。

### 仓库配置

- 根目录的个人 Markdown 文件和应用数据目录均由 Git 忽略，不会随代码发布。
- 添加 CC BY 4.0 许可证与署名说明；二次开发、再发布或商业使用时须保留 TopPlan、作者与项目链接，并说明修改内容。
